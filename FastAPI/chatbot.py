    ...
    if not is_relevant_result:
        ...
        general_prompt_template = ChatPromptTemplate.from_messages([
            ("system", (
                ...
                "당신은 주제({title})의 이름을 보고 어떤 카테고리인지 스스로 판단한 뒤, "
                "전달된 질문에 대해 '서울시 주거 맥락'에서 가장 정확한 정보를 제공해야 합니다. "
                "만약 [{title}]이 전문 용어라면 그 정의와 주거 생활에서의 의미를 설명하고, "
                "공고나 정책이라면 일반적인 절차나 혜택을 안내하세요.\n\n"
                "모든 답변은 마크다운 기호 사용을 해야합니다. 그래서 강조하거나 이모티콘을 넣어 가독성을 꼭 좋게 답변을 만드세요. "
                "답변은 짧아도 되지만 길면 최대 10문장까지 압축해서 가독성 쉽게 말을 만드세요"
                "검색된 공고 정보가 없어 질문이 정확하지 않거나 틀린 답변이 나올 수 있습니다. 이 문장은 답변 맨 마지막으로 고정.\n\n"
            )),
        ...
        prompt_template = ChatPromptTemplate.from_messages([
            ("system", (
                ...
                "모든 답변은 마크다운 기호 사용을 해야합니다. 그래서 강조하거나 이모티콘을 넣어 가독성을 꼭 좋게 답변을 만드세요. "
                "답변은 짧아도 되지만 길면 최대 10문장까지 압축해서 가독성 쉽게 말을 만드세요"
                "챗봇 형태이기 때문에 짧으면 한문장, 길면105문장 이내로 압축해서 핵심적이고 읽기 쉽게 만들어줘 마무리로"
            )),
            ("human", "내용:\n{context}\n\n질문: {question}")
        ])
