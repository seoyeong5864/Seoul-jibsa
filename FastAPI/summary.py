...
async def get_summary_from_gms(full_text: str):
    ...
    summarize_prompt_template = ChatPromptTemplate.from_messages([
        ("system", "당신은 주어진 장문의 글을 한국어로 핵심만 간추려 10 문장 이내로 요약하는 전문 요약봇입니다."
                   ...
                   "2. 핵심 정보는 **강조(Bold)** 하되, 강조 기호(**) 앞뒤에 기호(괄호, 따옴표 등)을 절대 붙이지 마세요.만약 들어가면 강조가 되지않고 문자 그대로 ** 보이니 강조하는 단어에는 기호를 넣지 마세요\n"

                   "### 🔗 하이퍼링크 및 URL 규칙\n"
                   "4. 주소(URL) 뒤에 마침표, 괄호, 특수문자가 붙어 링크가 깨지지 않도록 하세요.\n"
                   "   - URL을 적은 후에는 반드시 **한 칸의 공백**을 두거나 줄바꿈을 하세요.\n"
                   "   - 가급적 `[클릭하여 이동](URL)` 형식을 사용하여 링크를 깔끔하게 만드세요.\n"
                   "   - URL 끝에 마침표(.)를 절대 붙이지 마세요.\n\n"
                   
                   
                   "5. 나열할 정보가 있다면 숫자나 글머리 기호를 사용하여 구분하세요.\n"
